...
# Default-Stop:      0 1 6
# Short-Description: Stop/start nginx for service <%= @service_name %>
### END INIT INFO

# Author: Sergey Budnevitch <sb@nginx.com>

PATH=/sbin:/usr/sbin:/bin:/usr/bin
DESC="nginx service - <%= @service_name %>"
USER="<%= @nginx_user %>"
NAME=<%= @service_name %>
CONFFILE=<%= @nginx_conf_dir %>/nginx.conf
ROOT_DIR=<%= @nginx_root_dir %>
DAEMON=<%= @nginx_bin %>
DAEMON_ARGS="-c $CONFFILE -p $ROOT_DIR"
PIDFILE=<%= @nginx_tmp_dir %>/nginx.pid
...
do_upgrade() {
    ...
    if [ -f $OLDBINPIDFILE -a -f $PIDFILE ]; then
        start-stop-daemon --stop --signal QUIT --quiet --pidfile $OLDBINPIDFILE --exec $DAEMON 
        RETVAL="$?"
    else
        echo $"Upgrade failed!"
        RETVAL=1
